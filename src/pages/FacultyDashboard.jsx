...
const FacultyDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (profile?.primaryId) {
      conditionsD.push(where("assignedFacultyId", "==", profile.primaryId));
    }
    if (user?.uid) {
      conditionsD.push(where("assignedFacultyId", "==", user.uid));
    }

    if (conditionsD.length === 0) return;

    const qD = query(
      collection(db, "doubts"),
      conditionsD.length > 1 ? or(...conditionsD) : conditionsD[0]
    );
  ...
  const reassignDoubt = async (rejectedDoubt) => {
      ...
      const doubtContent = rejectedDoubt.title || rejectedDoubt.doubt || "";
                                ...
                                >
                                    <CardContent>
                                        ...
                                        <Typography mt={1} fontStyle="italic" color="text.primary"> 
                                            " {(d.description || d.doubt || "").substring(0, 70)}{(d.description || d.doubt || "").length > 70 ? '...' : ''} " 
                                        </Typography>
